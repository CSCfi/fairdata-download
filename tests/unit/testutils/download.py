...
def get_test_data(table_name, descriptor):
    test_data_file = "tests/unit/test_data/download_db/%s/%s.json" % (
        table_name,
        descriptor,
    )
...
def inject_database_records(flask_app, descriptor):
    ...
    with flask_app.app_context():
        ...
        if generate_scopes:
            ...
            dataset["files"] = list(map(lambda s: s["filepath"], generate_scopes))

        # Inject generate_request database rows
        generate_request = get_test_data("generate_request", descriptor)
        if generate_request:
            db_cursor.execute(
                "INSERT INTO generate_request (id, task_id) " "VALUES (?, ?)",
                (generate_request["id"], generate_request["task_id"]),
            )
