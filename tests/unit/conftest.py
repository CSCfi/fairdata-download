...
def mock_requests_get(monkeypatch):
    def skip_requesting_test(url):
        pytest.skip(
            "Test tried to request resource over network. This is not "
...
def mock_celery(monkeypatch, recorder, celery_task):
    ...
    monkeypatch.setattr("download.tasks.generate_task.delay", mock_generate_task)
...
@pytest.fixture
def success_not_outdated_task(flask_app):
    return create_dataset(flask_app, "success-not-outdated")
